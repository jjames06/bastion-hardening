...
|----|-------|------|---------|--------|
| **Windows 11 Pro** | **10.0.26200** (build **26200**) | 64-bit | **v15.9.4** | GPLv3; modular plain-text `src\` + MANIFEST; bat whoami-SID admin check (not `net session` — Server/LanmanServer off broke hardened re-launch); `tools-elevate-self.ps1`; Unblock-File + Process Bypass; script-scope load + command probe; modular Recovery; encrypted DNS/RDP undo; Settings-matching DoH; handbook/wiki; as of 2026-08-02 |
...
5. **Always use `Bastion-Hardening.bat`.** Never double-click `Bastion-Hardening.ps1` alone under the default Windows **Restricted** ExecutionPolicy - the host blocks the script before Bastion code runs ("running scripts is disabled" / UnauthorizedAccess).
   ...
   - The bat detects elevation with **whoami Administrators SID** (not `net session` — Bastion can disable the **Server** service / LanmanServer, which makes `net session` fail even when already admin and caused older bats to re-UAC-loop / flash-close)
