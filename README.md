...
4. Open that folder and confirm these files sit together:
   ...
   | `tools-run-bootstrap.ps1` | Yes in v15.9.5+ zip - Unblock + Process Bypass + start bootstrap |
   | `tools-elevate-self.ps1` | Yes in v15.9.5+ zip - UAC re-launch helper |
   | `src\` (modules + `MANIFEST.sha256`) | Yes - **plain-text** implementation; startup verifies hashes (integrity, not encryption) |
...
| Files "not found" after extract | Keep `.bat`, helpers, and `.ps1` in the **same** directory; keep `src\` next to them |
