...
.PARAMETER Version
  Version label without leading v (e.g. 16.0). Default: BASTION_RELEASE_VERSION env or 16.0.
...
.EXAMPLE
  pwsh -File tools/pack-release.ps1 -Version 16.0
#>
[CmdletBinding()]
param(
  [string]$Version = $(if ($env:BASTION_RELEASE_VERSION) { $env:BASTION_RELEASE_VERSION } else { "16.0" }),
  [string]$OutputDir = "",
