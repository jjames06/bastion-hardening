...
- **Two different “encrypted” ideas:** (1) Bastion **DPAPI** protects the snapshot file on disk. (2) Windows Settings **Encrypted** on DNS servers means **DNS-over-HTTPS (DoH)** on the network. Bastion Apply/restore enables DoH for known resolvers (Quad9, Cloudflare, Google, OpenDNS) when Windows accepts the template. Older snapshots that only stored IP addresses still restore IPs; DoH is re-applied from known templates when possible.  
