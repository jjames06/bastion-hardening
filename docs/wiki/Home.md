...
This wiki is a short **user handbook** for **Bastion v15.9.0** (modular plain-text `src\` builds). Deeper technical detail ships **in the release zip** under `docs/` and in **in-app Help** (main menu **11**). Download only from the [official site](https://www.operationlockedin.com/bastion/download) or the [Latest release](https://github.com/jjames06/bastion-hardening/releases/latest); avoid re-uploads from chat or third parties.
