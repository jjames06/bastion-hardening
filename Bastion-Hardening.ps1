...
<#
.SYNOPSIS
    Bastion Hardening Framework v15.0 FINAL
.DESCRIPTION
...
.NOTES
    Version 15.0 FINAL. System Restore is the strongest rollback. Run elevated. Save as UTF-8 (ASCII subset).
#>
        ...
        switch ($c.ToUpper()) {
            ...
            "U" {
                ...
                if ($targets.Count -eq 0) {
                    Write-Host "  Nothing selected. Toggle apps with numbers first, then press U." -ForegroundColor Red
                    Start-Sleep -Seconds 2
                    continue
                }
                Write-Host ""
                Write-Host "  Will uninstall:" -ForegroundColor Yellow
